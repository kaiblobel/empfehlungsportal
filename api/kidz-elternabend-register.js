...
async function registerParentEvening(secret, payload) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/register_kidz_elternabend_public`, {
    method: 'POST',
    headers: {
      apikey: ANON,
    ...
    body: JSON.stringify({
      ...
      p_time_preference: payload.timePreference,
      p_question: payload.question || null,
  ...
  if (!response.ok) {
    ...
    const error = new Error(message || `Supabase Elternabend: ${response.status}`);
...
module.exports = async function handler(req, res) {
  ...
  const registrationSecret = process.env.KIDZ_PARENT_EVENING_REGISTRATION_SECRET || '';
  ...
  const requestedTime = String(body.timePreference || '').trim().toLowerCase();
  const timePreference = ALLOWED_TIME_PREFERENCES.has(requestedTime) ? requestedTime : 'flexibel';
  ...
  try {
    ...
    const result = await registerParentEvening(registrationSecret, {
      name,
      email,
      telefon,
      source,
    ...
    if (!result?.ok || !result?.reference) throw new Error('Unvollständige Elternabend-Antwort');
    return send(res, 201, { ok: true, reference: result.reference });
  } catch (error) {
    console.error('[kidz-elternabend-register]', error.message);
