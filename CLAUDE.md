...
status: live v1.193 mit privatem Potenzialbuch (eigene RLS-geschuetzte Tabelle je Berater, keine Verbindung zu Empfehlungen, Promotern, Praemien, Benachrichtigungen oder Kennzahlen, bewusste manuelle Cockpit-Uebergabe), Sicherheits-Nachzug, mobile-first Portal und gefuehrter mobiler Praesentation. Phase 167 aktiv. VORBEREITET, NICHT LIVE: v1.194 / Phase 168 auf codex/potenzialbuch-kontaktstaerke mit Mehrfach-Kreisen, berechneter Kontaktstaerke von kalt bis sehr heiss, Symbolen, Kreis- und Staerkefiltern; die Migration schema-phase168.sql ist in der Live-DB bereits angewendet (Spalten am 10.08.2026 vorhanden), das zugehoerige Frontend ist weiter nicht live. NEU 10.08.2026 / v1.195 Phase 169: Telegram-Eingang zum Potenzialbuch live - Edge Function telegram-potenzial als Webhook des Portal-Bots, geteilte Kontakte, Textzeilen oder Sprachnachrichten (Groq-Whisper-Transkription plus LLM-Auswertung, GROQ_API_KEY in app_secrets) werden als Potenzial angelegt, Duplikat-Schutz, Chat-Knoepfe fuer Ziel Kunde/Partner und Loeschen, abgesichert ueber Webhook-Secret + Chat-Whitelist (nur Kais Chat), Berater-Zuordnung ueber app_secrets.TELEGRAM_BERATER_ID. NEU 10.08.2026 / v1.196 Phase 170: Team-Sichtbarkeit entlang der Fuehrungslinie live (nur Backend) - berater.fuehrungskraft_id plus rekursiver Helfer team_sichtbare_berater; team_metrics, team_activity_secure, team_activity und team_presence zeigen jedem Berater nur noch den eigenen Ast nach unten, Admins alles; Zuordnung: Josephine und Sven unter Kai, Sandro unter Sven, Max unter Sandro; live verifiziert (Sichtbarkeitsmatrix, Rechte-Check, team_metrics als Sandro). Zuordnung neuer Berater bisher nur per SQL, ein Fuehrungskraft-Feld in den Beraterkonten fehlt noch. OFFEN: echter eingeloggter Sichttest und Live-Freigabe fuer Phase 168. Der geleakte-Passwoerter-Schutz in Supabase Auth ist separat zu pruefen. NEU 11.08.2026 - Datenbestand scharf geschaltet: 15 als Test benannte Promoter mit 76 Empfehlungen, 10 Praemien und 3 Stufenmails geloescht, dazu ein Testkontakt im Potenzialbuch und eine KIDZ-Testteilnahme; die verfaelschte Kennzahlen-Historie (81 Tageswerte) wurde zurueckgesetzt und mit snapshot_kpis_today neu gestartet. Vollstaendige Sicherung als JSON im nicht exponierten Schema archiv, Tabelle backup_testdaten_20260811. Stand danach 23 Promoter und 14 Empfehlungen; die meisten der 14 stammen noch aus der Einrichtungsphase im Juni (Empfaenger sind die Berater selbst) und wurden bewusst behalten. Eine technische Trennung von Test und Echt (Kennzeichen, das aus den Kennzahlen herausgerechnet wird) existiert weiterhin NICHT.
live_url: https://empfehlungsportal.vercel.app
tags: portal, supabase, empfehlung, promoter, potenzialbuch, telegram, mobile-first, live, baufinanzierung, kfw, bookings, power-automate
-->
