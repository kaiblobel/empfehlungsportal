...
export async function listBerater() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('id, name, slug, email, rolle, telefon, foto_url, whatsapp, bookings_url, impressum_url, datenschutz_url, buero_foto_url, team_foto_url, buero_bildzeile, adresse, ist_aktiv, created_at, auth_user_id, fuehrungskraft_id, ist_test, cockpit_advisor_id')
      .order('created_at', { ascending: true });
    if (error) throw error;
    return { data: data || [], error: null };
  } catch (err) {
