...
test('Vorschauen koennen Produktionsvariablen nicht erben',()=>{
  const production={VERCEL_ENV:'production',BETRIEB_PORTAL_AKTIV:'1',BETRIEB_GEHEIMNIS_EMPFEHLUNG:'prod',BETRIEB_REDIS_URL:'prod'};
  ...
  const p=laufzeitUmgebung({...production,VERCEL_ENV:'preview',BETRIEB_VORSCHAU_AKTIV:'1',BETRIEB_VORSCHAU_HOST:'portal-probe.vercel.app',BETRIEB_VORSCHAU_GEHEIMNIS:'probe',BETRIEB_VORSCHAU_REDIS_URL:'probe'});
  assert.equal(p.aktiv,true);assert.equal(p.key,'probe');assert.equal(p.redisUrl,'probe');
