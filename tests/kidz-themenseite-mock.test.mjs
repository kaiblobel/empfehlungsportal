...
assert.match(javascript, /new URL\('\/kidz\/elternabend', window\.location\.origin\)/);
assert.match(javascript, /target\.searchParams\.set\('quelle', quelle\)/);
assert.match(javascript, /if \(berater\) target\.searchParams\.set\('berater', berater\)/);
assert.match(javascript, /#anmeldung/);
...
assert.match(html, /ohne dich schon für eine Teilnahme vorzumerken/);
...
assert.doesNotMatch(html, /FitBonus\+ Junior|künftige Arbeitskraft (?:Ihres|deines) Kindes|Arbeitskraft finanziell absichern/);
assert.match(html, /Gesundheitszustand früh sichern/);
assert.match(html, /Die U4 als früher Orientierungspunkt/);
assert.match(html, /VIP heißt bei KIDZ nicht Luxus/);
assert.equal((html.match(/data-car=/g) || []).length, 8);
assert.match(javascript, /const trainData = \{/);
assert.match(javascript, /label: 'Sicher im Kinderalltag'/);
assert.match(css, /\.train-viewport/);
assert.match(css, /\.loco-shape::before/);
assert.match(css, /\.pillar-original img \{ width: 190px; height: 190px;/);
assert.match(html, /Mehr Möglichkeiten statt mehr Produkte|mehr Möglichkeiten zu haben/);
assert.match(html, /pillar-mobile-instruction/);
assert.match(html, /Tippe auf eine Grundlage, um mehr zu erfahren/);
