...
export function portalPfad(pfad) {
  ...
  // Schraegstrich hinter einem Dateinamen (/hub.html/, /hub.html%2F, /hub.html/.) liefert Vercel als
  // dieselbe Datei aus. Deshalb gilt die Datei, nicht ein gedachtes index.html darunter (Abnahme 15.09.2026).
  if (p.endsWith('/')) {
    const ohne = p.replace(/\/+$/, '');
    p = ohne.split('/').at(-1).includes('.') ? ohne : p + 'index.html';
  }
